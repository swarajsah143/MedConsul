...
import { useAllotments, byRankRange } from '@/lib/allotment-data';
...
export default function EligibilityMatcherPage() {
  ...
  const { data: rows, loading, error, counsellings, filterOptions } = useAllotments();

  const [rankFrom, setRankFrom] = useState('');
  const [rankTo, setRankTo] = useState('');
  const [category, setCategory] = useState('All');
  const [state, setState] = useState('All');
  const [submitted, setSubmitted] = useState<{ from: number; to: number } | null>(null);
  ...
  const eligibleColleges = useMemo<EligibleCollege[]>(() => {
    if (!submitted) return [];
    let data = byRankRange(rows, submitted.from, submitted.to);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <select value={state} onChange={(e) => setState(e.target.value)} className={selectClass}>
                ...
                {counsellings.map((s) => <option key={s} value={s}>{s === 'All India Quota - MCC' ? 'All India Quota (MCC)' : s}</option>)}
              </select>
            </div>
          </div>
      ...
      {loading ? (
        ...
        rows.length === 0 ? (
          <EmptyState
            ...
            description="Allotment rows haven't been loaded for this platform yet, so there's nothing to match against."
          />
