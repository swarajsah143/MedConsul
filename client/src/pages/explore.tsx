...
const TYPE_STYLES: Record<string, string> = {
  ...
  AIIMS: 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400',
    ...
    <Card className="group h-full overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border-transparent hover:border-emerald-200 dark:hover:border-emerald-900/40 relative">
      ...
      <CardContent className="p-5">
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 pt-3">
          ...
          <div className="flex flex-wrap gap-1">
            {u.courses.slice(0, 6).map((c) => (
              <span key={c} className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-emerald-50/60 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400">
                {c}
              </span>
            ))}
            {u.courses.length > 6 && (
        ...
        <a
          ...
          className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:gap-2.5 transition-all"
        >
...
function BlogCard({ b }: { b: Blog }) {
  ...
  return (
    <a href={b.url} className="group block h-full">
      <Card className="h-full overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 border-transparent hover:border-emerald-200 dark:hover:border-emerald-900/40">
        <CardContent className="p-5 flex flex-col h-full">
          <span className="inline-flex items-center gap-1 self-start text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 mb-3">
            {b.category}
          </span>
          <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 leading-snug group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors line-clamp-2">
            {b.title}
