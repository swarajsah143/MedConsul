...
const QUICK_INFO_META: Record<string, { icon: LucideIcon; tint: string }> = {
  Type: { icon: Building2, tint: 'bg-red-50 text-red-600 dark:bg-red-950/40 dark:text-red-400' },
  ...
  'Annual Fees': { icon: IndianRupee, tint: 'bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400' },
            ...
            <Card className="sticky top-16 overflow-hidden">
              ...
              <CardContent className="p-5 space-y-4">
                <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                  <span className="w-6 h-6 rounded-lg gradient-primary flex items-center justify-center shadow-sm shadow-red-600/25">
                ...
                {courses.length > 0 && (
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {courses.map((c) => (
                        <span key={c} className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 rounded-full text-[10px] font-semibold text-slate-600 dark:text-slate-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/30 dark:hover:text-red-400 hover:scale-105 transition-all duration-200 cursor-default">{c}</span>
                      ))}
                    </div>
                  </div>
                )}
                ...
                {college.website && (
                  <div className="pt-1">
                    <Button asChild variant="outline" className="group/web w-full gap-2 text-xs rounded-xl h-10 hover:bg-red-50 hover:text-red-600 hover:border-red-200 dark:hover:bg-red-950/20 hover:shadow-sm hover:-translate-y-0.5 transition-all duration-200">
