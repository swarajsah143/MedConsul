 ...
 *
 * The decorative layers live in an absolutely-positioned, clipped layer so the
 * card's own `overflow` stays visible — children (e.g. dropdown menus) may
 * overflow the banner freely. Content should sit in `relative` markup; it
 * paints above the decor automatically.
 */
export function HeroBanner({
  ...
  return (
    <div
      className={cn(
        'group/hero relative rounded-3xl shadow-xl shadow-emerald-500/20 ring-1 ring-white/10',
        className
      )}
    >
      {/* Decoration layer — clipped to the rounded shape */}
      ...
      {/* Content */}
      <div className={cn('relative p-6 sm:p-8 lg:p-10', contentClassName)}>{children}</div>
    </div>
  );
}
